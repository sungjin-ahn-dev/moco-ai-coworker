...
class Settings(BaseSettings):
    ...
    # === agent-upgrades 고도화 플래그 (전부 기본 off = 라이브 무회귀; env로 토글) ===
    TOOL_RAG_ENABLED: bool = False        # 도구 네임스페이스 의도기반 축소 (⚠️ 미매핑 서버 드롭 위험 — 프로토타입)
    HYBRID_MEMORY_ENABLED: bool = False   # RRF 하이브리드 메모리 (register_semantic_ranker로 임베더 등록해야 실효)
    REFLEXION_ENABLED: bool = False       # 빈/약응답 반성 재시도 (⚠️ 전체 재실행→부작용 도구 재발화 가능, 멱등은 gmail만)
    IDEMPOTENCY_ENABLED: bool = False     # gmail_send 중복 전송 방지 (TTL 없음)
    PII_MASK_ENABLED: bool = False        # run_log 기록 PII 마스킹 (메모리 .md 는 미적용)
    TRACE_LOGGING_ENABLED: bool = False   # trace_id 상관 id (run_log.metadata; 로그 포맷 미변경 → 로그라인엔 안 찍힘)
    OUTBOX_ENABLED: bool = False          # 크래시세이프 아웃박스 (현재 큐 경로 미배선 — 모듈만)
    ORCH_MAX_TURNS: int = 0               # 0=무제한. 오케스트레이터 턴 상한 (SDK 지원 시 강제)
    SPEC_VERIFY_ENABLED: bool = False     # 응답 제출 전 요구사항 자기검증·보완 (spec_violation 저감 목적, +1턴)
    PMRV_ENABLED: bool = False            # Plan-Map-Reduce-Verify-Replan 코어 (복잡요청만; 단순=fast-path).
    #                                       Map=서브태스크 DAG를 execute_dag로 실행(독립 병렬·선행결과 TaskWorkspace 전달).
