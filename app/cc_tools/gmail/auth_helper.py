...
def _get_slack_user_email(slack_user_id: str) -> Optional[str]:
    ...
    try:
        ...
        logger.warning(f"[GMAIL] Failed to get Slack user email for {slack_user_id}: {e}")
    return None


def get_gmail_service(slack_user_id: Optional[str] = None):
    """
    Gmail API 서비스 객체 반환

    Args:
        slack_user_id: Slack 사용자 ID (예: 'U12345678')
                       지정 시 해당 사용자의 이메일로 Gmail에 접근 (Service Account)
    ...
    if oauth_credentials_path and os.path.exists(oauth_credentials_path):
        try:
            logger.info(f"[GMAIL] Using OAuth 2.0 Credentials: {oauth_credentials_path}")

            # 토큰 파일 경로 (Slack 사용자별 분리)
            if slack_user_id:
                token_filename = f"gmail_token_{slack_user_id}.json"
                logger.info(f"[GMAIL] Using per-user token for Slack user: {slack_user_id}")
            else:
                token_filename = "gmail_token.json"
                logger.info("[GMAIL] Using default token (no slack_user_id provided)")
            ...
            if os.path.exists(token_path):
                try:
                    ...
                    logger.info(f"[GMAIL] Loaded existing OAuth token from {token_filename}")
                except Exception as e:
            ...
            if not creds or not creds.valid:
                if creds and creds.expired and creds.refresh_token:
                    ...
                    logger.info(f"[GMAIL] Starting OAuth flow{user_msg} - 브라우저에서 로그인해주세요")
            ...
            logger.info("[GMAIL] OAuth 2.0 authentication successful")
            return service

        except Exception as e:
            logger.warning(f"[GMAIL] OAuth 2.0 authentication failed: {e}, trying Service Account...")
    ...
    if service_account_path and os.path.exists(service_account_path):
        try:
            ...
            if slack_user_id:
                ...
                if user_email:
                    logger.info(f"[GMAIL] Auto-mapped Slack user {slack_user_id} to email: {user_email}")

            # Slack에서 이메일을 못 가져오면 설정값 사용 (fallback)
            if not user_email:
                user_email = settings.GMAIL_USER_EMAIL
                if user_email:
                    logger.info(f"[GMAIL] Using fallback email from settings: {user_email}")

            if not user_email:
                raise ValueError(
                    "Gmail 사용자 이메일을 확인할 수 없습니다. "
                    "Slack 프로필에 이메일이 설정되어 있는지 확인하거나, "
                    "GMAIL_USER_EMAIL 설정값을 지정해주세요."
                )

            # 캐시 확인
            cache_key = ("gmail", user_email)
            if cache_key in _service_cache:
                logger.debug(f"[GMAIL] Using cached service (user={user_email})")
                return _service_cache[cache_key]

            logger.info(f"[GMAIL] Using Service Account: {service_account_path}")
            logger.info(f"[GMAIL] Delegating to user: {user_email}")
            ...
            logger.info("[GMAIL] Service Account authentication successful (cached)")
            return service

        except Exception as e:
            logger.error(f"[GMAIL] Service Account authentication failed: {e}")
            raise ValueError(f"Gmail 인증 실패: {str(e)}")
