...
async def gmail_send_message(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # [idempotency] opt-in: 동일 (수신자·제목·본문) 중복 전송 방지(기본 off·무회귀)
        _idem_key = None
        _idem_store = None
        try:
            from app.config.settings import get_settings as _gs
            if getattr(_gs(), "IDEMPOTENCY_ENABLED", False):
                from app.cc_utils.idempotency import make_key, IdempotencyStore
                _idem_key = make_key("gmail_send_message", {
                    "slack_user_id": slack_user_id, "to": to, "subject": subject,
                    "body": body, "cc": cc, "bcc": bcc})
                _idem_store = IdempotencyStore(os.path.join(
                    os.environ.get("FILESYSTEM_BASE_DIR", "."), "idempotency", "gmail_send.jsonl"))
                _cached = _idem_store.seen(_idem_key)
                if _cached is not None:
                    _c = _cached if isinstance(_cached, dict) else {}
                    return {"content": [{"type": "text", "text": json.dumps(
                        {"success": True, "idempotent": True,
                         "message": f"중복 전송 방지(이미 발송됨): {to}", **_c},
                        ensure_ascii=False, indent=2)}]}
        except Exception:
            _idem_key = None
            _idem_store = None
        ...
        if _idem_key is not None and _idem_store is not None:
            try:
                _idem_store.record(_idem_key, _payload)
            except Exception:
                pass
        ...
        return {
            "content": [{
                "type": "text",
                "text": json.dumps(_payload, ensure_ascii=False, indent=2)
            }]
        }
