...
# [trace] opt-in: 표준 로깅에 trace_id 필터 부착(기본 off·무회귀)
try:
    from app.config.settings import get_settings as _gs_boot
    if getattr(_gs_boot(), "TRACE_LOGGING_ENABLED", False):
        from app.cc_utils.trace import install_trace_logging
...
async def main():
    ...
    async def orchestrator_wrapper(job, client):
        ...
        # [trace] opt-in: 요청 단위 trace_id 바인딩(기본 off → '-'·무회귀). run_log metadata로 로그↔런 조인.
        _trace_id = "-"
        try:
            from app.config.settings import get_settings as _gs_tr
            if getattr(_gs_tr(), "TRACE_LOGGING_ENABLED", False):
                from app.cc_utils.trace import new_trace_id, bind_trace_id
        ...
        try:
            ...
            # [eval] 도구 궤적 수집 → run_log.tools_used + 온라인 tool-accuracy 활성화
            _tool_trace: list = []
            def _collect_tools(_msg):
                for _b in getattr(_msg, "content", []) or []:
                    if getattr(_b, "type", "") == "tool_use" or _b.__class__.__name__ == "ToolUseBlock":
                        _n = getattr(_b, "name", "")
                        if _n:
                            _tool_trace.append(_n)
            ...
            # [eval fix] 실패도 반드시 기록 — 기존엔 성공만 로깅돼 성공률이 항상 100%였음
            try:
                from app.cc_utils.run_log_store import run_log as _rl
                import time as _tm2
                _lv = locals()
                _rl.log_run(
                    run_type="operator",
                    ...
                    user_name=job["message_data"].get("user_name", ""),
                    channel_id=channel_id, thread_ts=thread_ts or "",
                    prompt=job.get("query", ""), response="",
                    tools_used=_lv.get("_tool_trace", []),
                    state=("timeout" if "timeout" in str(_orch_err).lower() else "error"),
                    error=str(_orch_err),
                    elapsed_seconds=(_tm2.time() - _lv["_orch_start"]) if "_orch_start" in _lv else 0.0,
                    metadata={"trace_id": _lv.get("_trace_id", "-")},
                )
            except Exception:
                pass
            raise
