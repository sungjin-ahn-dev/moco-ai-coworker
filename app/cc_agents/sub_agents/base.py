...
def log_cache_usage(message, agent_name: str) -> None:
    """서브에이전트 ResultMessage 의 usage 로 프리픽스 캐시 히트율을 로깅한다.

    각 서브에이전트는 별도 claude CLI 서브프로세스라, Anthropic usage 의
    cache_read/creation/input 으로 '호출별 프리픽스 캐시 히트율'을 잰다(자체서빙 불필요).
    측정·로깅이 실패해도 요청 흐름을 절대 깨지 않는다(무해).
    """
    try:
        u = getattr(message, "usage", None)
        if not u:
            return
        from app.cc_eval.metrics import cache_efficiency
        e = cache_efficiency(u)
        hr = e["hit_rate"]
        sv = e["est_cost_saving"]
        logging.info(
            "[CACHE_USAGE] %s: hit_rate=%.1f%% (read=%d create=%d input=%d) est_saving=%.1f%%"
            % (agent_name,
               (hr * 100 if hr == hr else 0.0),          # hr==hr : NaN 방어
               e["cache_read_tokens"], e["cache_creation_tokens"], e["input_tokens"],
               (sv * 100 if sv == sv else 0.0))
        )
    except Exception:
        pass
