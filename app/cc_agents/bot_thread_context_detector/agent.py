...
이 메시지가 대상 "{bot_name}"에게 추가 질의하는 것입니까?"""
        ...
        async with RetryableSDKClient(options, max_retries=3, agent_name="THREAD_CONTEXT") as sdk_client:
            await sdk_client.query(query)

            async for message in sdk_client.receive_response():
                if isinstance(message, ResultMessage):
                    result_text = message.result.strip().lower()
                    logging.info(f"[BOT_THREAD_CONTEXT] Claude response: {result_text}")
