...
def _maybe_mask_pii(text: str) -> str:
    """opt-in PII 마스킹(PII_MASK_ENABLED, 기본 off·무회귀)."""
    try:
        from app.config.settings import get_settings
        if getattr(get_settings(), "PII_MASK_ENABLED", False):
...
class RunLogStore:
    ...
    def log_run(
        ...
        entry = {
            ...
            "thread_ts": thread_ts,
            "prompt": _maybe_mask_pii(prompt)[:500],        # 프롬프트 500자 제한(+opt-in PII 마스킹)
            "response": _maybe_mask_pii(response)[:500],    # 응답 500자 제한(+opt-in PII 마스킹)
